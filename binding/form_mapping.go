...
type setter interface {
	TrySet(value reflect.Value, field reflect.StructField, key string, set setOptions) (isSet bool, err error)
}
type formSource map[string][]string

var _ setter = formSource(nil)

func (form formSource) TrySet(value reflect.Value, field reflect.StructField, tagValue string, set setOptions) (bool, error) {
	return setByForm(value, field, form, tagValue, set)
}
...
func setIntField(val string, bitsSize int, field reflect.Value) error {

	if val == "" {
		val = "0"
	}
	intVal, err := strconv.ParseInt(val, 10, bitsSize)
	if err == nil {
		field.SetInt(intVal)
	}
	return err
...
func setFloatField(val string, bitSize int, field reflect.Value) error {
	...
	float32Val, err := strconv.ParseFloat(val, bitSize)
	if err == nil {
		field.SetFloat(float32Val)
	}
	return err
}
